...
const char* mqtt_server = "192.168.110.104";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if ((char)payload[0] == '1') {
    digitalWrite(D6Pin, LOW);   // Turn the LED on (Note that LOW is the voltage level
    // but actually the LED is on; this is because
    // it is active low on the ESP-01)
  } else {
    digitalWrite(D6Pin, HIGH); // Turn the LED off by making the voltage HIGH
  }
